...
abstract class ShuffleSuite extends SparkFunSuite with Matchers with LocalSparkContext {
  ...
  test("shuffle non-zero block size") {
    ...
    (0 until NUM_BLOCKS).foreach { id =>
      val statuses = SparkEnv.get(sc._sparkUser).mapOutputTracker.getMapSizesByExecutorId(shuffleId, id)
  ...
  test("zero sized blocks without kryo") {
    ...
    val blockSizes = (0 until NUM_BLOCKS).flatMap { id =>
      val statuses = SparkEnv.get(sc._sparkUser)
        .mapOutputTracker.getMapSizesByExecutorId(shuffleId, id)
      statuses.flatMap(_._2.map(_._2))
    }
    val nonEmptyBlocks = blockSizes.filter(x => x > 0)
