...
object SharedState extends Logging {
  ...
  Try { URL.setURLStreamHandlerFactory(new FsUrlStreamHandlerFactory()) } match {
    case Success(_) =>
    case Failure(ex) =>
      logWarning("Failed to set FsUrlStreamHandlerFactory with URL.setURLStreamHandlerFactory", ex)
  }
